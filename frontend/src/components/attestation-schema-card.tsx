...
const AttestationsSchemaCard = ({ schemaId }: { schemaId: string }) => {
  ...
  return (
    <Link
      href={`${networkBaseUrl(chainMapper(account.chain?.name))}/${schemaId}`}
      target="_blank"
    >
      <div className="my-3 flex flex-row items-center justify-center rounded-lg border px-4 py-4 text-left">
        ...
        <Link
          href={`${networkBaseUrl(chainMapper(account.chain?.name))}/attestation/attestWithSchema/0x41a4762b6d24720f43720883824687b54a3263b38b82d0ad93a0dd3d47024728`}
        ...
        <Link
          href={`${networkBaseUrl(chainMapper(account.chain?.name))}/attestation/attestWithSchema/0x6b837e8a2baca15566c0231e7050cd76738771d2fea0ea4e1bd94cc94a851b47`}
